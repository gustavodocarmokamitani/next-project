...
import {
  confirmarPresencaAtletaGerente,
  cancelarPresencaAtletaGerente,
  registrarPagamentoAtletaGerente,
} from "./actions"
...
export function AtletasEventoContent({
  ...
  const handleConfirmPresence = async () => {
    ...
    try {
      ...
      await confirmarPresencaAtletaGerente(evento.id, selectedAtleta.id, items)
      router.refresh()
    } catch (error) {
  ...
  const handleCancelPresence = async (atleta: AtletaEventoDTO) => {
    ...
    setLoading(true)
    try {
      await cancelarPresencaAtletaGerente(evento.id, atleta.id)
      router.refresh()
    } catch (error) {
  ...
  const handleRegisterPayment = async () => {
    ...
    try {
      ...
      await registrarPagamentoAtletaGerente(evento.id, selectedAtleta.id, items)
      router.refresh()
    } catch (error) {
  ...
  if (success) {
    setTimeout(() => {
      router.replace(`/gerente/eventos/${evento.id}/atletas`)
    }, 3000)
  }
